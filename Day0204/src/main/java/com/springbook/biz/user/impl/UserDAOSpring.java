...
public class UserDAOSpring extends JdbcDaoSupport{
   ...
   getJdbcTemplate().update(USER_INSERT,user.getId(),user.getPassword(),user.getName(),user.getRole());
			}
	...
	public int getUserCnt(UserVO vo) {
		...
		return  getJdbcTemplate().queryForInt(USER_CNT,vo.getId());
	}
	...
	public UserVO getUser(UserVO user) throws SQLException{
		...
		try {
		     user =getJdbcTemplate().queryForObject(USER_GET, args, new UserRowMapper());
