...
public class GetBoardController /* implements Controller */ {
     ...
     public String getBoard(BoardVO vo, Model model, BoardDAO boardDAO) {
		 ...
		 * HttpServletResponse response) { System.out.println("글 상세 조회 처리");
		 * 
		 ...
		 * 
		 * // 3. 검색 결과를 ModelAndView 에 저장하고 상세 화면을 리턴 //HttpSession session =
		 * request.getSession();// //session.setAttribute("board", board); //return
		 * "getBoard";
		 * 
		 * //이동할 경로가 ~.jsp 이므로 view명만 지정 ModelAndView mav = new
		 * ModelAndView("getBoard"); mav.addObject("board", board);//세션에 저장하던 정보를 ->
		 * ModelAndView에 저장 return mav; }
		 */
